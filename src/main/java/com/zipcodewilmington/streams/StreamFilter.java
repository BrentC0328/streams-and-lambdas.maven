...
public class StreamFilter {
    ...
    public StreamFilter() {
        ...
        this.startingCharacter = RandomUtils.createString('a', 'z', 1);
    }
    ...
    public List<Person> toListMultiLine() {
      ...
      String name = "";
                this.personStream.forEach(person -> {
                            if (person.getName().indexOf(this.startingCharacter) == 0) {
                                result.add(person);
                            }
                        }
                );
    ...
    public List<Person> toListOneLine() {
        List<Person> result = new ArrayList<>();
        this.personStream.forEach(person -> { if (person.getName().indexOf(this.startingCharacter) == 0) result.add(person); });
        return result;
    }
